    ...
    : QMainWindow(parent)
{
   ...
   bar = new QStatusBar(this);
    ...
    myicon.addFile(tr(":/image/1.jpg")); //让QIcon对象指向想要的图标
    ui.pushButton_5->setIcon(myicon); //给按钮添加图标
    ui.pushButton_5->setIconSize(QSize(102, 102));//重置图标大小
    ui.pushButton_5->setStyleSheet("background-color:rgba(0,0,0,0)");
    ...
    ui.pushButton->setIconSize(QSize(102, 102));//重置图标大小
...
void MainWindow::camera_calobration()
{
    ...
    QDir dir(desktop_path+ QString::fromLocal8Bit("/结果"));
    if (!dir.exists())
    {
        dir.mkdir(desktop_path + QString::fromLocal8Bit("/结果"));
    }
    QDir dir1(desktop_path + QString::fromLocal8Bit("/结果/相机标定"));
    if (!dir1.exists())
    {
        dir1.mkdir(desktop_path + QString::fromLocal8Bit("/结果/相机标定"));
    }
    //QFileInfo fileinfo(desktop_path + QString::fromLocal8Bit("/结果/相机标定/result.txt"));
   
    ofstream ofs; 
    string path = desktop_path.toLocal8Bit().toStdString() + "/结果/相机标定/result.txt";
